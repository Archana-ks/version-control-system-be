...
const register = async (req, res) => {
  try {
    ...
    newUser.save((err, data) => {
      if (err) {
        return res.status(400).send({ message: "This email is already registered. If you don't remember the password, try to reset the password in the login page or try to register with another email.", error: err });
      }

...
const addRepo = async (req, res) => {
  try {
    ...
    newRepo.save((err, data) => {
      if (err) {
        return res.status(400).send({ message: "Error while creating the repository", error: err });
      }
  ...
  try {
    ...
    if (existingRepo) {
      ...
      Repos.findByIdAndUpdate(repoId, { $push: { version: payload } }, (err, data) => {
          if (err) { 
            return res.status(400).send({ message: "Error while committing the changes", error: err });
          }
        }
      );
